...
use Kagiamy\GLGFWin\main;

class Main extends PluginBase implements Listener{
     
     public function onEnable(){
          $this->getServer()->getPluginManager()->registerEvents($this,$this);
          $this->getLogger()->info("Stats by Kagiamy enabled!");
          $this->saveResource("config.yml");
          $this->config = new Config($this->getDataFolder(). "config.yml", Config::YAML);
          if($this->getServer()->getPluginManager()->getPlugin("EconomyAPI") != null){
               $this->economyapi = $this->getServer()->getPluginManager()->getPlugin("EconomyAPI");
               $this->getLogger()->notice("EconomyAPI support in Stats enabled!");
          }else{
               $this->economyapi = null;
               $this->getLogger()->notice("EconomyAPI support in Stats disabled!");
          }
          if($this->getServer()->getPluginManager()->getPlugin("GLGFWin") != null){
               $this->glgfwin = $this->getServer()->getPluginManager()->getPlugin("GLGFWin");
               $this->getLogger()->notice("GLGFWin support in Stats enabled!");
          }else{
               $this->glgfwin = null;
               $this->getLogger()->notice("GLGFWin support in Stats disabled!");
          }
     }
     ...
     public function getStats($player, $ptype = "me"){
          ...
          if($this->glgfwin != null){
               $stats = str_replace("{wins}", $this->glgfwin->getwins($player->getName()), $stats);
          }
     ...
     public function onCommand(CommandSender $sender, Command $command, $label, array $args){
          switch($command->getName()){
               case "stats":
                    if($sender instanceof Player && !isset($args[0]) && $sender->hasPermission("stats.my")){
